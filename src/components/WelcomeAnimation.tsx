...
const DEFAULT_STROKE_WIDTH = 35;
const DEFAULT_ANIMATION_DURATION = 3; // seconds

// Constants for the "hello" animation
...
// Properly calculated viewBox based on path bounds
...
const WelcomeAnimation: React.FC<WelcomeAnimationProps> = ({ 
  ...
  // Auto complete after animation duration plus a small delay
  useEffect(() => {
    const timer = setTimeout(() => {
      onComplete();
    }, (duration * 1000) + 500); // Animation duration + 500ms extra for visual effect
      ...
      >
        <svg 
          ...
          style={{
            width: 'auto',
            maxWidth: '90%',
            maxHeight: isSmallScreen ? '70vh' : '50vh',
            height: 'auto',
            overflow: 'visible',
          }}
          aria-label={text}
